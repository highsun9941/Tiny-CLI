...
Tiny-CLI는 모델에게 **코딩 작업에 필요한 최소한의 도구 목록만** 추가로 노출합니다. 사용자의 입력, 현재 세션의 대화와 도구 결과를 전달하며, 자체 시스템 프롬프트나 저장소 지침을 자동으로 주입하지 않습니다. 계획, 실행 순서, 문제 진단과 재시도는 모델이 결정합니다.
...
기본 모델 요청에는 사용자 대화, 이전 모델 응답, 도구 결과와 아래 네 가지 도구만 들어갑니다.

| 도구 | 동작 |
| --- | --- |
| `read_file` | UTF-8 파일 읽기 |
| `write_file` | 파일 쓰기와 부모 디렉터리 생성 |
| `replace_in_file` | 정확히 한 번 일치하는 문자열 교체 |
| `run_command` | 셸 명령 실행 및 종료 코드·출력 반환 |

모델 응답 → 도구 호출 실행 → 결과 전달을 반복하며, 모델이 도구 호출 없이 응답하면 해당 턴을 마칩니다. 파일 경로와 셸 실행은 작업 디렉터리에 제한되지 않습니다.

- 자체 system/developer 프롬프트가 없습니다.
- `AGENTS.md`, 메모리, 스킬, 저장소 지도 등을 자동으로 읽어 넣지 않습니다. 모델은 필요하면 파일 도구로 직접 읽을 수 있습니다.
...
테스트는 모의 HTTP 전송을 사용해 기본 프롬프트 부재, 네 가지 도구, 도구 오류 반환, 제공자 프로필 선택, OpenAI/Anthropic 도구 호출 왕복, 플러그인 선택과 UI 세션 전환을 검증합니다. 실제 API 호출이나 API 키가 필요하지 않습니다.
